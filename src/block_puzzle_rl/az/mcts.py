...
class MCTS:
    def __init__(
        ...
        self.heuristic_weights: Dict[str, float] = {
            ...
            "almost": 0.5,
            # New heuristics
            "mobility": 0.002,          # per legal placement after
            "windows2x2": 0.5,          # count of empty 2x2 windows
            "singletons": 1.0,          # penalty per interior 1x1 pocket
        }
        ...
        for a in legal_idxs.tolist():
            ...
            if not q.get("valid", False):
                ...
                s = (
                    ...
                    + w["mobility"] * float(q.get("mobility_after", 0))
                    + w["windows2x2"] * float(q.get("windows2x2_after", 0))
                    - w["singletons"] * float(q.get("singletons_after", 0))
                )
            scores[a] = float(s)
        ...
        for _ in range(self.n_simulations):
            ...
            while not node.terminal:
                ...
                if not ok:
                    ...
                    break
                path.append((node, a, float(gained)))
                ...
                if a not in node.children:
                    ...
                    # Backup
                    G = float(v)
                    for parent, act, rew in reversed(path):
                        G = rew + self.gamma * G
